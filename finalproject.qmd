...
Because variables such as genre, rating, and country likely have some variance in their effect on the model across their categories, we decided to create random effects for these variables. Our fixed effects are ones which have constant effects on the model which doesn't change for different "categories", namely budget, score, votes, and runtime. We created this table to summarize our results from linear mixed model (since the original output is too long)  
```{r random-effect, message = F, warning = F, echo = F}
data = data.frame(Groups=c('country', 'genre', 'rating', 'residual'), 
                  variance=c(0.20, 0.08, 1.16, 1.85),
                  std=c(0.44, 0.29, 1.07, 1.36))
...
```
```{r random-effect, message = F, warning = F, echo = F}
data = data.frame(Groups=c('budge', 'score', 'votes', 'runtime'), 
                  estimate=c(0.16, 0.15, 0.002,0.006),
                  stddev=c(0.06, 0.02, 0.00, 0.01),
                  tvalue=c(27.114, 5.411, 20.319, 5.041))
