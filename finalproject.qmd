...
library(lme4)
m4 <- lm(log_gross ~ log(budget) + genre + rating + score + log(votes) + log(runtime) + country, data = data)
m5 <- lmer(log_gross ~ 1 + log(budget) + (1 | genre) + (1 | rating) + score + log(votes) + log(runtime) + (1 | country), data = data)
summary(m4)$coef
summary(m5)
