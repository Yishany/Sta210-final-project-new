...
Based on the above violations, we want to apply linear mixed model to our datasets to further explore our questions. We want to look at the associations between our interested variables, and randomeffect due to country. 

```{r random-effect, message = F, warning = F}
library(lme4)
m4 <- lm(log_gross ~ budget + genre + rating + score + votes + year + runtime + country, data = data)
m5 <- lmer(log_gross ~ budget + genre + rating + score + votes + year + runtime + (1 | country), data = data)
summary(m4)$coef
summary(m5)$coef
```
While adjusting for the random intercept based on country variable and holding all other variables constant, we noticed that XXXXX. ???????? (not just coefficient explanation?????????)
...
Explanation for LASSO: Based on this, XXXX variables are included in our model. 
