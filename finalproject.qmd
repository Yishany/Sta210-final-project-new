...
```{r linear-reg-variables, message = F, warning = F, echo = F, result = F}
...
## Sensitivity Matrix
We're curious about the sensitivity, specificity, positive and negative predicted values.
...
In our predictor variables there are likely to be correlated variables that weren’t fully considered (particularly any interaction effect beyond two-way, for example, between budget, votes, rating, and score there might’ve been a four-way interaction effect we could have included). Though including such interaction effects might’ve led to highly increased model complexity, leading to overfitting, we did not thoroughly examine the potential presence of overfitting in our project (based on our low number of predictor variables to observations (7 to 5000+). 
