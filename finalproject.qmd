...
```{r numeric-variable-distributions, message = F, warning = F, echo = F}
ggplot(data = data) + 
...
```{r interesting-relationships, message = F, warning = F, echo = F}
data2 <- data %>% 
...
```{r correlation-matrix, message = F, warning = F, echo = F}
...
As mentioned above we are interested in the relationships between the variables and success, but we still wish to further filter variables that are more likely to have a significant effect on our model. Therefore, we will apply a LASSO variable selection process in order to determine significant variables to explore.
```{r lasso, message = F, warning = F, echo = F, results = F}
set.seed(919)
library(glmnet)
y <- data$log_gross
...
```{r linear-regression-variables, message = F, warning = F, echo = F}
...
```{r random-effect, message = F, warning = F, echo = F}
library(lme4)
...
```{r interact-score, message = F, warning = F, echo = F}
...
```{r interact-votes, message = F, warning = F, echo = F}
library(lme4)
r.rmse4 <- sqrt(mean(residuals(m4)^2))
r.rmse5 <- sqrt(mean(residuals(m5)^2))
r.rmse4
...
```{r prediction-gross, message = F, warning = F, echo = F}
set.seed(123)
dim(data)
